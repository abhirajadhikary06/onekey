...
async def send_api_alert_email(email: str, provider: str, status_code: int, response_text: str, model: str = None, endpoint: str = None):
    ...
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: auto; border: 1px solid #e2e8f0; border-radius: 10px; padding: 20px;">
        <h2 style="color: {color}; border-bottom: 2px solid {color}; padding-bottom: 10px;">Onekey API Alert: Failure</h2>
        <p>A request to <strong>{provider}</strong> has failed with status <strong>{status_code}</strong>.</p>
        ...
        <pre style="background-color: #fef2f2; border: 1px solid #fee2e2; color: #991b1b; padding: 10px; border-radius: 5px; overflow-x: auto; font-size: 11px;">
{cleaned_response[:2000]}
        </pre>
        
        <p style="font-size: 12px; color: #64748b; margin-top: 20px;">
            This is an automated error alert from your Onekey server.
        </p>
    </div>
    """

    message = MessageSchema(
        subject=f"Onekey Alert: {provider} Failure ({status_code})",
        recipients=[email],
        body=html_content,
